...
getLastLoc = function(host, port, useragent)
  ...
  response.last_location = false

  if response.location then
    response.last_location = response.location[#response.location] or false
  end
...
action = function(host, port)
  ...
  local default_user_agent = "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.57 Safari/537.17"
  local loc = getLastLoc(host, port, default_user_agent)

  local allowed, forb, default = {}, {}, {}

  default.status = tostring(loc.status)
  default.last_location = loc.last_location or ""
  default.user_agent = default_user_agent
  ...
  for _, l in ipairs(HTTPlibs) do
    ...
    if loc.last_location ~= libloc.last_location then
      local msg = l .. " redirected to: " .. tostring(libloc.last_location)
      local libhost = http.parse_url(tostring(libloc.last_location))
  ...
  if next(default) ~= nil then
    table.insert(default, 1, "Default Request:")
  end

  return {default, allowed, forb}
