...
docker build --no-cache -t cs/aucote/release .

if test "${branch#*tags/}" != "$branch"; then
	VERSION="${branch#tags/}"
else
	SHORT_COMMIT=`expr substr $GIT_COMMIT 1 7`
	VERSION="0.0.0-`date -u +"%Y%m%d%H%M%S"`-$SHORT_COMMIT-$destEnv"
fi
...
curl -X POST $APTLY_SERVER/api/repos/main/file/aucote_$VERSION
