...
class SslDhParams(NmapScript):
    ...
    def handle_script(self, script):
        ...
        state = script.find("./table[@key='NMAP-2']/elem[@key='state']").text
        log.debug('State of port: %s', state)
        if state != 'VULNERABLE': return None
        vulner = Vulnerability()
        vulner.title = script.find("./table[@key='NMAP-2']/elem[@key='title']").text
        vulner.description = script.find("./table[@key='NMAP-2']/table[@key='description']/elem").text
