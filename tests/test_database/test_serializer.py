...
class SerializerTest(TestCase):
    ...
    def setUp(self):
        ...
        port.scan = Scan()
        port.scan.start = datetime.datetime(2016, 8, 16, 15, 23, 10, 183095)
        port.when_discovered = datetime.datetime(2016, 8, 16, 15, 23, 10, 183095)
        ...
        self.vuln.when_discovered = datetime.datetime(2016, 8, 16, 15, 23, 10, 183095)

    def test_vulnerability_serializer(self):

        msg = self.serializer.serialize_port_vuln(self.vuln.port, self.vuln)

        self.assertEqual(msg.data, bytearray(b'\x00\x00\xe7\x1e\x85\x93V\x01\x00\x00\x16\x00 \x02\x7f\x00\x00\x01\x00'
                                              b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x03\x00ssh\x00\x00'
                                              b'\x00\x00\x06\xe7\x1e\x85\x93V\x01\x00\x00\x04\x00Test\x01\x00\x00\x00'
                                              b'\xe7\x1e\x85\x93V\x01\x00\x00'))
