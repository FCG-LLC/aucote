...
steps = [
    step("CREATE TABLE scans(rowid SERIAL UNIQUE, protocol int, scanner_name VARCHAR, scan_start int, "
         "scan_end int, UNIQUE (protocol, scanner_name, scan_start))"),

    step("CREATE TABLE nodes_scans(rowid SERIAL UNIQUE, scan_id int, node_id BIGINT, node_ip text, "
         "time int, primary key (scan_id, node_id, node_ip))"),

    step("CREATE TABLE ports_scans (rowid SERIAL UNIQUE, scan_id int, node_id BIGINT, node_ip text, "
         "port int, port_protocol int, time int, primary key (scan_id, node_id, node_ip, port, port_protocol))"),

    step("CREATE TABLE security_scans (rowid SERIAL UNIQUE, scan_id int, exploit_id int, "
    ...
    step("CREATE TABLE vulnerabilities(rowid SERIAL UNIQUE, scan_id int, node_id BIGINT, "
    ...
    step("CREATE TABLE changes(rowid SERIAL PRIMARY KEY, type int, vulnerability_id int, "
